...
import { QUESTIONS, CATEGORIES, DIFF_LABELS, CAT_COLORS, type Question } from "@/data/questions";
...
export default function Home() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({data:{session}})=>{
      if(session?.user){ const u=session.user; setUser({id:u.id,email:u.email??undefined,name:u.user_metadata?.full_name||u.user_metadata?.name||u.email?.split("@")[0],avatar:u.user_metadata?.avatar_url,login:u.user_metadata?.user_name||u.user_metadata?.preferred_username}); }
      setAuthLoading(false);
    });
    const {data:{subscription}}=supabase.auth.onAuthStateChange((_e,session)=>{
      if(session?.user){ const u=session.user; setUser({id:u.id,email:u.email??undefined,name:u.user_metadata?.full_name||u.user_metadata?.name||u.email?.split("@")[0],avatar:u.user_metadata?.avatar_url,login:u.user_metadata?.user_name||u.user_metadata?.preferred_username}); } else setUser(null);
    });
    return ()=>subscription.unsubscribe();
  }, []);

  const signInWithGitHub=async()=>{ await supabase.auth.signInWithOAuth({provider:"github",options:{redirectTo:window.location.origin}}); };
  const signOut=async()=>{ await supabase.auth.signOut(); setUser(null); setRecords(new Map()); setBookmarks(new Set()); setDropdownOpen(false); };
  ...
  const pickRandom=useCallback((cat="all",diff="all",review=false)=>{
    ...
    const f=unseen.length>0?unseen:pool;
    return f[Math.floor(Math.random()*f.length)];
  },[bookmarks,completed,activeQuestions]);

  const startStudy=(q:Question,isReview=false)=>{ setCurrentQ(q); const prev=records.get(q.id); setAnswer(prev?.answer||""); setFeedback(null); setReviewMode(isReview); setView("study"); };
  ...
  const saveRecord=async(qId:number,ans:string,fb:string)=>{
    ...
    if(user){ await upsertStudyRecord(user.id,qId,ans,fb); }
    else { const all=[...records.values()]; all.push(rec); const m=new Map<number,StudyRecord>(); all.forEach(r=>m.set(r.question_id,r)); saveJSON("guestRecords",[...m.values()].map(r=>({qid:r.question_id,answer:r.answer,feedback:r.feedback,date:r.studied_at}))); }
    const today=getToday();
    if(lastStudyDate!==today){ const ns=streak+1; setStreak(ns); setLastStudyDate(today); saveJSON("streak",ns); saveJSON("lastStudyDate",today); }
  };

  const toggleBookmarkAction=async(qId:number)=>{
    const nb=new Set(bookmarks);
    if(nb.has(qId)){nb.delete(qId);if(user) removeBookmark(user.id,qId);} else {nb.add(qId);if(user) addBookmark(user.id,qId);}
    setBookmarks(nb); if(!user) saveJSON("bookmarks",[...nb]);
  };
  ...
  const requestFeedback=async()=>{
    ...
    } catch { setFeedback("네트워크 오류가 발생했습니다."); }
    setLoadingFeedback(false);
  };

  const openBrowse=(mode:BrowseMode)=>{ setBrowseMode(mode); setFilterCat("all"); setFilterDiff("all"); setExpandedId(null); setView("browse"); };

  if(!dataLoaded||authLoading) return <div className="min-h-screen flex items-center justify-center text-zinc-500">Loading...</div>;
  ...
  const ProfileArea=()=>(
    <div className="relative" ref={dropdownRef}>
      {user?(
        <>
          <button onClick={()=>setDropdownOpen(!dropdownOpen)} className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-white/[0.06] transition-all cursor-pointer bg-transparent border-none">
            {user.avatar&&<img src={user.avatar} alt="" className="w-8 h-8 rounded-full border border-white/10"/>}
            <div className="text-left hidden sm:block"><div className="text-sm font-semibold text-zinc-300 max-w-[100px] truncate">{user.login||user.name}</div></div>
          ...
          {dropdownOpen&&(
            <div className="absolute right-0 top-full mt-2 w-56 bg-[#18181b] border border-white/10 rounded-xl shadow-2xl z-50 overflow-hidden">
              ...
              <div className="border-t border-white/[0.06] py-1">
                <button onClick={signOut} className="w-full px-4 py-2.5 text-left text-sm text-red-400 hover:bg-white/[0.06] cursor-pointer bg-transparent border-none">로그아웃</button>
              </div>
            </div>
          )}
        </>
      ):(
        <button onClick={signInWithGitHub} className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-zinc-200 bg-white/[0.06] border border-white/10 rounded-lg hover:bg-white/[0.1] transition-all cursor-pointer">
      ...
      <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={()=>setShowCategorySettings(false)}>
        <div className="bg-[#18181b] border border-white/10 rounded-2xl w-full max-w-md max-h-[80vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
          <div className="px-5 py-4 border-b border-white/[0.06] flex items-center justify-between">
            ...
            <button onClick={selectAllCategories} className="text-xs text-indigo-400 cursor-pointer bg-transparent border-none">전체 선택</button>
          </div>
          <div className="p-4 space-y-1.5">
            {CATEGORIES.map(cat=>{const active=selectedCategories.has(cat);const color=CAT_COLORS[cat]||"#888";const count=QUESTIONS.filter(q=>q.category===cat).length;return(
              <button key={cat} onClick={()=>toggleCategory(cat)} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all cursor-pointer border-none text-left ${active?"bg-white/[0.06]":"bg-transparent opacity-40"}`}>
                <span className="w-3 h-3 rounded-full flex-shrink-0" style={{background:active?color:"#555"}}/><span className={`text-sm font-medium flex-1 ${active?"text-zinc-200":"text-zinc-500"}`}>{cat}</span><span className="text-xs text-zinc-500">{count}문제</span>
                <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center ${active?"border-indigo-400 bg-indigo-400":"border-zinc-600"}`}>{active&&<svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7"/></svg>}</div>
              </button>);})}
          </div>
          <div className="px-5 py-4 border-t border-white/[0.06]"><button onClick={()=>setShowCategorySettings(false)} className="w-full py-3 text-sm font-bold text-white bg-gradient-to-r from-indigo-500 to-violet-500 rounded-xl cursor-pointer border-none">완료 ({selectedCategories.size}개 선택)</button></div>
        </div>
      </div>);};
    ...
    <div className="min-h-screen"><CategoryModal/><ApiKeyModal/>
      <div className="max-w-[640px] mx-auto px-5 py-6 pb-16">
        <div className="flex items-start justify-between mb-7">
          <div><h1 className="text-[28px] font-extrabold bg-gradient-to-r from-indigo-400 to-purple-400 bg-clip-text text-transparent tracking-tight">CS 면접 마스터</h1><p className="mt-1.5 text-sm text-zinc-500">매일 한 문제, 백엔드 면접 준비</p></div>
          <ProfileArea/>
        </div>
        <div className="flex gap-3 mb-5">
          <div className="flex-1 bg-gradient-to-br from-orange-500/10 to-amber-500/8 border border-amber-500/20 rounded-2xl p-4 text-center"><div className="text-[28px] animate-fire">🔥</div><div className="text-[32px] font-extrabold text-amber-400 leading-none">{streak}</div><div className="text-xs text-zinc-400 mt-1">일 연속</div></div>
          <button onClick={()=>openBrowse({type:"today"})} className="flex-1 bg-white/[0.03] border border-white/[0.08] rounded-2xl p-4 text-center cursor-pointer hover:bg-white/[0.06] transition-all"><div className="text-2xl font-bold text-zinc-200 mt-2">{todayCount}</div><div className="text-xs text-zinc-500 mt-1">오늘 학습</div></button>
          <button onClick={()=>openBrowse({type:"completed"})} className="flex-1 bg-white/[0.03] border border-white/[0.08] rounded-2xl p-4 text-center cursor-pointer hover:bg-white/[0.06] transition-all"><div className="text-2xl font-bold text-zinc-200 mt-2">{completedInActive}/{totalQ}</div><div className="text-xs text-zinc-500 mt-1">완료</div></button>
        </div>
        <div className="flex items-center gap-3 mb-7"><div className="flex-1 h-2 bg-white/[0.06] rounded-full overflow-hidden"><div className="h-full bg-gradient-to-r from-indigo-400 to-purple-400 rounded-full progress-fill" style={{width:`${progress}%`}}/></div><span className="text-sm text-zinc-400 font-semibold whitespace-nowrap">{progress}% 달성</span></div>
        <button onClick={()=>{const q=pickRandom();if(q) startStudy(q);}} className="w-full py-[18px] text-[17px] font-bold text-white bg-gradient-to-r from-indigo-500 to-violet-500 rounded-[14px] hover:-translate-y-0.5 hover:shadow-[0_8px_30px_rgba(99,102,241,0.4)] transition-all cursor-pointer">⚡ 오늘의 랜덤 문제 풀기</button>
        {bookmarkCount>0&&(<button onClick={()=>openBrowse({type:"bookmarks"})} className="w-full mt-3 py-3.5 text-[15px] font-semibold text-amber-400 bg-amber-400/[0.08] border border-amber-400/25 rounded-xl hover:bg-amber-400/15 transition-all cursor-pointer">🔖 북마크 모아보기 ({bookmarkCount}문제)</button>)}
        <div className="mt-8 mb-3.5 flex items-center justify-between"><span className="text-base font-bold text-zinc-400">카테고리별 학습</span><button onClick={()=>setShowCategorySettings(true)} className="text-xs text-zinc-500 hover:text-zinc-300 cursor-pointer bg-transparent border-none">설정 ⚙️</button></div>
        <div className="grid grid-cols-3 gap-2.5 mb-5">
          {CATEGORIES.map(cat=>{const active=selectedCategories.has(cat);const catQs=QUESTIONS.filter(q=>q.category===cat);const catDone=catQs.filter(q=>completed.has(q.id)).length;const pct=Math.round((catDone/catQs.length)*100);const color=CAT_COLORS[cat]||"#888";return(
            <button key={cat} onClick={()=>{if(active)openBrowse({type:"category",category:cat});}} className={`flex flex-col items-start gap-1 p-3 rounded-xl transition-all text-left border ${active?"bg-white/[0.03] border-white/[0.08] hover:bg-white/[0.06] cursor-pointer":"bg-white/[0.01] border-white/[0.04] opacity-35 cursor-default"}`} style={{borderColor:active?color+"33":undefined}}>
              <span className="w-2 h-2 rounded-full" style={{background:active?color:"#555"}}/><span className={`text-[13px] font-semibold ${active?"text-zinc-200":"text-zinc-600"}`}>{cat}</span><span className="text-xs text-zinc-500">{catDone}/{catQs.length}</span>
              <div className="w-full h-[3px] bg-white/[0.06] rounded-full overflow-hidden mt-0.5"><div className="h-full rounded-full transition-all" style={{width:`${pct}%`,background:active?color:"#555"}}/></div>
            </button>);})}
        </div>
  ...
  // ━━━ BROWSE ━━━
  if(view==="browse"){
    let basePool=activeQuestions; let browseTitle="전체 질문"; let showFilters=true;
    ...
    if(showFilters){if(filterCat!=="all") filtered=filtered.filter(q=>q.category===filterCat);if(filterDiff!=="all") filtered=filtered.filter(q=>q.difficulty===Number(filterDiff));}
    return(
      <div className="min-h-screen"><div className="max-w-[640px] mx-auto px-5 py-6 pb-16">
        ...
        <h2 className="text-[22px] font-extrabold flex items-center gap-2.5 mb-4">{browseTitle}<span className="text-sm font-medium text-zinc-500">{filtered.length}문제</span></h2>
        {filtered.length===0&&(<div className="text-center py-16"><div className="text-4xl mb-3">{browseMode.type==="today"?"📚":browseMode.type==="bookmarks"?"🔖":"✨"}</div><p className="text-zinc-400 text-sm">{browseMode.type==="today"?"아직 오늘 학습한 문제가 없어요.":browseMode.type==="bookmarks"?"아직 북마크한 문제가 없어요.":browseMode.type==="completed"?"아직 완료한 문제가 없어요.":"조건에 맞는 문제가 없어요."}</p><button onClick={()=>{const q=pickRandom();if(q)startStudy(q);}} className="mt-4 px-6 py-2.5 text-sm font-semibold text-indigo-400 bg-indigo-400/[0.08] border border-indigo-400/20 rounded-lg cursor-pointer hover:bg-indigo-400/15 transition-all">랜덤 문제 풀기</button></div>)}
        {showFilters&&filtered.length>0&&(<div className="flex gap-2.5 mb-5">{browseMode.type!=="category"&&(<select value={filterCat} onChange={e=>setFilterCat(e.target.value)} className="flex-1 px-3.5 py-2.5 text-sm bg-white/[0.05] border border-white/10 rounded-[10px] text-zinc-200 outline-none cursor-pointer"><option value="all">전체 카테고리</option>{CATEGORIES.map(c=><option key={c} value={c}>{c}</option>)}</select>)}<select value={filterDiff} onChange={e=>setFilterDiff(e.target.value)} className="flex-1 px-3.5 py-2.5 text-sm bg-white/[0.05] border border-white/10 rounded-[10px] text-zinc-200 outline-none cursor-pointer"><option value="all">전체 난이도</option><option value="1">기초</option><option value="2">중급</option><option value="3">심화</option></select></div>)}
        <div className="flex flex-col gap-3">
          {filtered.map(q=>{const isDone=completed.has(q.id);const isBk=bookmarks.has(q.id);const rec=records.get(q.id);const isExpanded=expandedId===q.id;return(
            <div key={q.id} className="bg-white/[0.03] border border-white/[0.07] rounded-[14px] p-4">
              <div className="flex items-center gap-2 mb-2.5 flex-wrap">
                <DiffBadge difficulty={q.difficulty}/><span className="text-xs font-semibold" style={{color:CAT_COLORS[q.category]}}>{q.category}</span>
                {isDone&&rec&&<span className="text-xs text-zinc-500">{formatDate(rec.studied_at)}</span>}
                <button onClick={()=>toggleBookmarkAction(q.id)} className="ml-auto text-lg bg-transparent border-none cursor-pointer leading-none" style={{color:isBk?"#fbbf24":"#555"}}>{isBk?"★":"☆"}</button>
              </div>
              <p className="text-sm leading-relaxed text-zinc-300 mb-3">{q.question}</p>
              <div className="flex items-center gap-2">
                <button onClick={()=>startStudy(q)} className="text-[13px] font-semibold text-indigo-400 bg-indigo-400/[0.08] border border-indigo-400/20 rounded-lg px-4 py-2 hover:bg-indigo-400/15 transition-all cursor-pointer">{isDone?"다시 학습하기":"학습하기"}</button>
                {isDone&&rec?.answer&&(<button onClick={()=>setExpandedId(isExpanded?null:q.id)} className="text-[13px] font-semibold text-zinc-500 bg-white/[0.04] border border-white/[0.08] rounded-lg px-4 py-2 hover:bg-white/[0.08] transition-all cursor-pointer">{isExpanded?"접기 ▲":"내 답변 보기 ▼"}</button>)}
              </div>
  ...
  if(view==="study"&&currentQ){
    ...
    const handleComplete=async()=>{await saveRecord(currentQ.id,answer,feedback||"");setView("home");};
    const handleNext=async()=>{if(answer.trim()) await saveRecord(currentQ.id,answer,feedback||"");const q=reviewMode?pickRandom("all","all",true):pickRandom();if(q) startStudy(q,reviewMode);else setView("home");};
    return(
      <div className="min-h-screen"><ApiKeyModal/>
        <div className="max-w-[640px] mx-auto px-5 py-6 pb-16">
          <button onClick={()=>setView("home")} className="text-indigo-400 text-sm font-semibold mb-4 cursor-pointer bg-transparent border-none">← 홈으로</button>
          ...
          <div className="bg-gradient-to-br from-indigo-500/[0.08] to-violet-500/[0.05] border border-indigo-500/20 rounded-[18px] p-6 mb-6">
            <div className="flex items-center gap-2 mb-4"><DiffBadge difficulty={currentQ.difficulty}/><span className="text-xs font-semibold" style={{color:CAT_COLORS[currentQ.category]}}>{currentQ.category}</span><button onClick={()=>toggleBookmarkAction(currentQ.id)} className="ml-auto text-[22px] bg-transparent border-none cursor-pointer leading-none" style={{color:isBk?"#fbbf24":"#666"}}>{isBk?"★":"☆"}</button></div>
          ...
          <div className="mb-5">
            ...
            <textarea value={answer} onChange={e=>setAnswer(e.target.value)} placeholder="면접처럼 답변을 작성해보세요..." rows={6} className="w-full px-4 py-4 text-[15px] leading-[1.7] text-zinc-200 bg-white/[0.04] border border-white/10 rounded-[14px] outline-none resize-y min-h-[140px]"/>
